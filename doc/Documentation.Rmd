...
The parameters will be explained consecutively:
  ...
  + -f _filename_, --file=_filename_
    ...
    For the columns the file has to contain a header. For the rows it can, but doesn't need 
    to contain an identifier in the first column. columns have to be tab-seperated.
    
    If you don't set this parameter, BEclearCL expects this input from STDIN, so that it 
  ...
  + -o _filename_, --out=_filename_
     ...
     If you don't set this parameter, BEclearCL prints the output to STDOUT.
  ...
  + -s _filename_ --samples=_filename_
    ...
    If this parameter is nor provided each sample is treated as its own batch.
  ...
  + -b _filename_, --BEscore=_filename_
    ...
    If not set, the file isn't written.
